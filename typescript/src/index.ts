...
export function soloMayusculas(nombres: string[]): string[] {
  return nombres.map (s => s.toUpperCase());
}
...
export function promedioAprobados(
  ...
  return aprobados.length === 0 
    ? 0 
    : aprobados.reduce((acc, e) => acc + e.nota, 0) / aprobados.length;
}
...
export function totalVentasCredito(
  ...
  return transacciones
    .filter(t => t.tipo === "credito" && t.monto > 100)
...
export function procesarVentas(
  ...
  const filtradas = ventas.filter(v => v.monto > 100);
  const total = filtradas.reduce((acc, v) => acc + v.monto, 0);
...
export function transformarDatos(
  ...
  return registros
    ...
    .map(r => ({
      nombre: r.nombre,
      promedio: r.ventas.length === 0
        ? 0
        : Math.round((r.ventas.reduce((acc,v) => acc + v, 0) / r.ventas.length) * 100) / 100
    }))
    .slice()
